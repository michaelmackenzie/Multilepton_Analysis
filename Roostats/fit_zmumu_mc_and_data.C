//Fit the background templates to the oppositie- and same-sign MC, and same-sign data
...
const char* get_fig_dir() {
  ...
  return Form("plots/latest_production/%s/zmumu_mc_data_fits_%i%s", years_s.Data(), set_, tag_.Data());
}
...
//---------------------------------------------------------------------------------------------------------------------------------------
// Main function
int fit_zmumu_mc_and_data(int set = 13, vector<int> years = {2016,2017,2018}) {
  ...
  RooDataHist zmumu_hist("zmumu_hist", "Z->#mu#mu", RooArgList(obs), h_zmumu);
  const double ndata = data->Integral();
  const double nmc   = h_zmumu->Integral();
  ...
  RooAbsPdf* zmm_pdf = create_zmumu    (obs, set, false); zmm_pdf->SetName("zmm_pdf"); zmm_pdf->SetTitle("Z->#mu#mu PDF");
  RooRealVar* n_bkg = new RooRealVar("n_bkg", "N(bkg)"      , 1., 0., 1.5*ndata);
  RooRealVar* n_zmm = new RooRealVar("n_zmm", "N(Z->#mu#mu)", 1., 0., 1.5*ndata);
  RooAddPdf* tot_pdf = new RooAddPdf("data_pdf", "Total PDF", RooArgList(*zmm_pdf, *bkg_pdf), RooArgList(*n_zmm, *n_bkg));
  ...
  cat.defineType("data");
  cat.defineType("mc"  );
  RooDataHist totalData("totalData","total data", obs, Index(cat), Import("data", data_hist),Import("mc", zmumu_hist));

  // Construct a simultaneous pdf for data and MC
  RooSimultaneous totalPDF("totalPDF", "Data and MC PDF", cat);
  totalPDF.addPdf(*tot_pdf,"data");
  ...
  //Plot the results
  {
    ...
    c->SaveAs(Form("%s/fit_data.png", get_fig_dir()));
    delete frame;
    ...
    //Compare the fit to the total Z->mumu MC
    pad1->cd();
    frame = obs.frame();
    zmumu_hist.plotOn(frame);
    zmm_pdf->plotOn(frame);
    nparams = zmm_pdf->getParameters(zmumu_hist)->getSize();
    nentries = zmumu_hist.numEntries();
    ndof = nentries - nparams;
    frame->Draw();
    h_diff = data_pdf_diff(&zmumu_hist, zmm_pdf, obs);
    chi_sq = get_chi_square_from_diff(h_diff);
    label.DrawLatex(0.60, 0.87, Form("#chi^{2}/DOF = %.2f; p = %.3f", chi_sq/ndof, TMath::Prob(chi_sq, ndof)));
    cout << "OS MC: chi_sq = " << chi_sq << ", ndof = " << ndof << endl;
    label.DrawLatex(0.53, 0.82, Form("#mu = %.2f#pm%.2f; #sigma = %.2f#pm%.2f",
    ...
    c->SaveAs(Form("%s/fit_mc.png", get_fig_dir()));
    delete frame;
