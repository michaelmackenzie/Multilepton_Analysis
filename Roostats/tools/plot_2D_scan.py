...
y_range = [1.e10, -1.e10]
for ev in t:
    x.append(getattr(ev, param_1))
    y.append(getattr(ev, param_2))
    deltaNLL.append(getattr(ev, "deltaNLL"))
    x_range[0] = min(x_range[0], x[-1])
    x_range[1] = max(x_range[1], x[-1])
    y_range[0] = min(y_range[0], y[-1])
    y_range[1] = max(y_range[1], y[-1])
...
canv.Update()
canv.SaveAs("%s/scan_2d_%s_vs_%s.png" % (plot_dir, param_1, param_2))
