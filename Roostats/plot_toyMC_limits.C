...
TCanvas* plot_toyMC_limits(int set = 8, int year = 2016) {
  ...
  c->Divide(2,2);
  c->cd(1);
  TH1F* hlim = new TH1F("hlim", "Observed upper limits", 50, 1.e-9, 1.e-6);
  tree->Draw("upperLimit>>hlim", "", "E");
  hlim->SetLineWidth(2);
  hlim->SetMarkerStyle(20);
  auto pad = c->cd(2);
  TH1F* hexp = new TH1F("hexp", "Expected upper limits (-1, 0, +1)", 50, 1.e-9, 1.e-6);
  tree->Draw("expectedLimit>>hexp");
  hexp->SetLineWidth(2);
  TH1F* hexpLo = new TH1F("hexpLo", "hexplo", 50, 1.e-9, 1.e-6);
  tree->Draw("plusLimit>>hexpLo", "", "sames");
  hexpLo->SetLineWidth(2);
  hexpLo->SetLineColor(kGreen);
  hexpLo->SetLineStyle(kDashed);
  TH1F* hexpHi = new TH1F("hexpHi", "hexpHi", 50, 1.e-9, 1.e-6);
  ...
  c->cd(3);
  TH1F* hdiff = new TH1F("hdiff", "Observed limit - Expected limit", 50, -1e-6, 1.e-6);
  tree->Draw("(upperLimit-expectedLimit)>>hdiff", "" ,"E");
  hdiff->SetLineWidth(2);
  hdiff->SetMarkerStyle(20);
  c->cd(4);
  TH1F* hpull = new TH1F("hpull", "Observed limit pulls", 100, -3, 3);
  tree->Draw("((upperLimit-expectedLimit)/((upperLimit>expectedLimit) ? (minusLimit-expectedLimit) : (expectedLimit-plusLimit)))>>hpull", "", "E");
