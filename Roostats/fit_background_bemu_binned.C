...
Int_t fit_background_bemu_binned(int set = 8, TString selection = "zemu",
  ...
  RooDataHist bkgData ("bkgData" , "Background Data", RooArgList(lepm), hbkg );
  RooDataHist sigData ("sigData" , "Signal Data"    , RooArgList(lepm), hsig );
  RooDataHist dataData("dataData", "Data Data"      , RooArgList(lepm), hdata);
  ...
  RooRealVar eff_nominal("eff_nominal", "Nominal signal efficiency", hsig->Integral()/(lum*xs_sig_), 0., 1.);
  eff_nominal.setConstant(1);
  cout << "Global variables defined!\n";


  ////////////////////////////////////////////////////////////
  // Fit signal distribution
  ////////////////////////////////////////////////////////////

  cout << "--- Performing the signal fit!\n";
  ...
  RooRealVar mean2("mean2", "mean2", lepm.getVal(), lepm.getMin(), lepm.getMax());
  ...
  RooAddPdf sigPDF("sigPDF", "signal PDF", sigpdf1, sigpdf2, fracsig);

  RooRealVar N_sig("N_sig", "N_sig", 2e5, 1e2, 3e6);

  RooAddPdf totsigpdf("totsigpdf", "Signal PDF", RooArgList(sigPDF), RooArgList(N_sig));
  ...
  RooFormulaVar n_sig("n_sig",  "@0*@1*@2*@3", RooArgList(br_sig_eff, lum_var, bxs_var, eff_nominal));
  ...
  RooProdPdf totPDF_constr     ("totPDF_constr"     , "totPDF_constr"     , RooArgList(totPDF     , br_sig_constr));
  RooProdPdf totPDF_constr_exp ("totPDF_constr_alt" , "totPDF_constr_alt" , RooArgList(totPDF_exp , br_sig_constr));
  RooProdPdf totPDF_constr_cheb("totPDF_constr_cheb", "totPDF_constr_cheb", RooArgList(totPDF_cheb, br_sig_constr));
  RooProdPdf totPDF_constr_pow ("totPDF_constr_pow" , "totPDF_constr_pow" , RooArgList(totPDF_pow , br_sig_constr));
  RooProdPdf totPDF_constr_land("totPDF_constr_land", "totPDF_constr_land", RooArgList(totPDF_land, br_sig_constr));
  RooProdPdf totPDF_constr_bst2("totPDF_constr_bst2", "totPDF_constr_bst2", RooArgList(totPDF_bst2, br_sig_constr));
  RooProdPdf totPDF_constr_bst3("totPDF_constr_bst3", "totPDF_constr_bst3", RooArgList(totPDF_bst3, br_sig_constr));
  RooProdPdf totPDF_constr_bst4("totPDF_constr_bst4", "totPDF_constr_bst4", RooArgList(totPDF_bst4, br_sig_constr));
  RooProdPdf totPDF_constr_bst5("totPDF_constr_bst5", "totPDF_constr_bst5", RooArgList(totPDF_bst5, br_sig_constr));
  ...
  c1->SaveAs(Form("plots/latest_production/%s/fit_%s_lepm_binned_background_%i.png", year_string.Data(), selection.Data(), set));

  //save the workspace
  TFile* fOut = new TFile(Form("workspaces/fit_%s_lepm_background_binned_%s_%i.root", selection.Data(), year_string.Data(), set), "RECREATE");
  fOut->cd();
