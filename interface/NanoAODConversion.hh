...
namespace CLFV {
  ...
  public :
    ...
    Float_t leptonOneDXY  = 0          ;
    Float_t leptonOneDZ   = 0          ;
    Float_t leptonTwoDXY  = 0          ;
    Float_t leptonTwoDZ   = 0          ;
    ...
    Float_t zLepOnePt          = 0.           ;
    Float_t zLepTwoPt          = 0.           ;
    Float_t zLepOneEta         = 0.           ;
    Float_t zLepTwoEta         = 0.           ;
    ...
    void    InitializeOutBranchStructure(TTree* tree);
    void    InitializeInBranchStructure(TTree* tree);
    float   BTagWeight(int WP);
    void    InitializeEventVariables();
    void    InitializeTreeVariables(Int_t selection);
    void    CountJets(int selection);
    void    CountTaus(int selection);
    void    CountLightLeptons(int selection);
    void    ApplyMuonCorrections();
    bool    ElectronID(Int_t index, Int_t ID);
    bool    MuonID(Int_t index, Int_t isoID, Int_t ID);
    bool    TauID (Int_t index, Bool_t useDeep, Int_t antiEle, Int_t antiMu, Int_t antiJet, Bool_t DM, Int_t oldAntiMu);
    void    CountObjects();
    bool    SelectionID(Int_t selection);
    int     QCDSelection(Int_t selection);
    float   GetTauFakeSF(int genFlavor);
    float   CorrectMET(int selection, float met);
    float   GetZPtWeight(float pt);
    void    SwapLeptons();
    ...
    Int_t         fIsEmbed = 0; //is an embedding sample or not
