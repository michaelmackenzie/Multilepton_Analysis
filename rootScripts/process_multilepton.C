...
Int_t process_multilepton() {
  ...
  for(int i = 0; i < sizeof(files)/sizeof(*files); ++i) {
    ...
    TFile* f = new TFile(Form("%s%s",
			      (doPtStudyFiles == 0) ? gridPath[i]:gridPathPtStudy[i],c));
    if(f == 0) {
    ...
    tree->Process(selec,"");
    TFile* out = new TFile(Form("%s.hist",tree->GetName()),"UPDATE");
    if(out == 0) {
      printf("Unable to find output hist file %s.hist, continuing\n",tree->GetName());
      continue;
    }
    events->Write();
    out->Write();
    ...
    if(doPtStudyFiles > 0 ) {
      int res = rename(Form("%s.hist",tree->GetName()),Form("%s.ptstudy_hist",tree->GetName()));
      if(res == 0) printf("Renamed file to %s.ptstudy_hist\n",tree->GetName());
      else         printf("Failed to rename file to %s.ptstudy_hist\n",tree->GetName());
    }
