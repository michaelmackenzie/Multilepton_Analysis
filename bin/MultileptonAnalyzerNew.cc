#include "MultileptonAnalyzerNew.h"
...
MultileptonAnalyzerNew::MultileptonAnalyzerNew() : BLTSelector()
{

}

MultileptonAnalyzerNew::~MultileptonAnalyzerNew()
{

}

void MultileptonAnalyzerNew::Begin(TTree *tree)
{
...
Bool_t MultileptonAnalyzerNew::Process(Long64_t entry)
{
    ...
    if(!isData) {
      ...
      if (genWeight < 0) {
	hTotalEvents->Fill(10); //for normalization, removing negative events
      }
    } else {
      genWeight = 1.;
    }
    ...
    for (int i=0; i < fMuonArr->GetEntries(); i++) {
        ...
        if (
	        muon->pt > min(min(minIsoMuonSubPt, minVetoPt),minLeadingPt) //ensure all needed are included
                && fabs(muon->eta) < 2.4
                // tight muon ID
    ...
    for (unsigned i = 0; i < tmp_muons.size(); i++) {
	  ...
	  if ((doTriggerPt == 0 || muonP4.Pt() > minVetoPt) && fabs(muonP4.Eta()) < 2.1) {
                veto_muons.push_back(muonP4);

                if (doTriggerPt == 0 || muonP4.Pt() > minLeadingPt || (muons.size() > 0 && muonP4.Pt() > minTrailingPt)) {
	...
	} else if(muons.size() > 0 && (vJet.DeltaR(muons[0]) >= 0.5)) { //for trigger/pt studies, only veto on main muons
	  unsigned index = 0; //find second muon
	  for (unsigned i = 1; i < muons.size(); ++i) {
	    if (muons_q[0] != muons_q[i]) { //opposite signed muons
	      index = i;
	      break;
	    }
	  }
	  if(vJet.DeltaR(muons[index]) < 0.5) muOverlap = true;
	} else if (muons.size() > 0){
	  muOverlap = true; //overlaps with first muon
	}
        ...
        if (
            ...
            if (fabs(jet->eta) <= 2.4) { 
                if (
 		        jet->pt > 30.  // not needed since above requires this
                        && jet->mva > -0.89
                        && !muOverlap 
                        //&& !elOverlap
                   ) { 
    ...
    if (bjets.size() > 1 || requireBTag == 0) {
      int startIndex = (requireBTag == 0) ? 0 : 1;
      jets.insert(jets.end(), bjets.begin()+startIndex, bjets.end());
    }
    ...
    if (params->selection == "mumu") {
      if (muons.size() < 2 || (require2Leptons > 0 && muons.size() != 2))
            return kTRUE;
        hTotalEvents->Fill(5);
        ...
        muonOneP4 = muons[0];
        for (unsigned i = 1; i < muons.size(); ++i) {
	  if(require2Leptons > 0) { muonTwoP4 = muons[i]; muonTwoIndex = i; break;}
	  if (muons_q[0] != muons_q[i]) {
        ...
        if (
	    (muonOneP4.Pt() <= minLeadingPt && doTriggerPt > 0) || !(fabs(muonOneP4.Eta()) < 2.1)
	    || (muonTwoP4.Pt() <= minTrailingPt && doTriggerPt > 0) || !(fabs(muonTwoP4.Eta()) < 2.1)
           )
            return kTRUE;
        hTotalEvents->Fill(6);
        ...
        if (dimuon.M() < minLeptonMass || dimuon.M() > maxLeptonMass)
            return kTRUE;
        hTotalEvents->Fill(7);
	...
	if ((requireBTag > 0 && nBJets == 0) || (nBJets + nJets < 2 && nFwdJets == 0) ||
	    (requireNoOverlapCategories == 1 && (nBJets + nJets > 1 && nFwdJets > 0)))
            return kTRUE;
        hTotalEvents->Fill(8);
	...
	if (printAll) {  
	  ...
	  std::cout << "nJets " << nJets << " nBJets " << nBJets
        ...
        if ((doTriggerPt > 0 && muons[0].Pt() < 25.) || fabs(muons[0].Eta()) > 2.1)
            return kTRUE;
        hTotalEvents->Fill(6);
    ...
    } else { // no selection selected, should be done at Begin Job stage?
      return kTRUE;
    }
    ...
    if (bjets.size() > 0 && requireBTag > 0) {
        bjetP4.SetPtEtaPhiM(bjets[0]->pt, bjets[0]->eta, bjets[0]->phi, bjets[0]->mass);
        bjetD0     = bjets[0]->d0;
        bjetTag    = bjets[0]->csv;
        bjetPUID   = bjets[0]->mva;
        bjetFlavor = bjets[0]->hadronFlavor;
    } else if (jets.size() > 1 && requireBTag == 0) {
        bjetP4.SetPtEtaPhiM(jets[1]->pt, jets[1]->eta, jets[1]->phi, jets[1]->mass);
        bjetD0     = jets[1]->d0;
        bjetTag    = jets[1]->csv;
        bjetPUID   = jets[1]->mva;
        bjetFlavor = jets[1]->hadronFlavor;
    } else if (jets.size() == 1 && requireBTag == 0 && fwdjets.size() > 0) {
        bjetP4.SetPtEtaPhiM(jets[0]->pt, jets[0]->eta, jets[0]->phi, jets[0]->mass);
        bjetD0     = jets[0]->d0;
        bjetTag    = jets[0]->csv;
        bjetPUID   = jets[0]->mva;
        bjetFlavor = jets[0]->hadronFlavor;
    } else {
...
void MultileptonAnalyzerNew::Terminate()
{
    outFile->Write();
    outFile->Close();
...
void MultileptonAnalyzerNew::ReportPostBegin()
{
...
void MultileptonAnalyzerNew::ReportPostTerminate()
{
...
{
    std::unique_ptr<MultileptonAnalyzerNew> selector(new MultileptonAnalyzerNew());
...
float MultileptonAnalyzerNew::MetKluge(float met)
{
    if (met > 500) {
        return 1.;
    }
