...
ElectronIDWeight::ElectronIDWeight(int Mode, int seed, int verbose) : verbose_(verbose) {
  ...
  for(int period = k2016; period < (k2018 + 1); ++period) {
    ...
    f = TFile::Open((path + "/" + electronTrigFileNames[period].first).Data(), "READ");
    if(f) {
...
double ElectronIDWeight::interpolate(TH2F* h, double eta, double pt) {
...
//-------------------------------------------------------------------------------------------------------------------------
double ElectronIDWeight::TriggerEff(double pt, double eta, int year, float& data_eff, float& mc_eff) {
